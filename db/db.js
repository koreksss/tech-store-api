const Database = require("better-sqlite3");   //исп. для работы с бд SQLite
const path = require("path");     //подкл. встроенного модуля path

const dbPath = path.join(__dirname, "store.db");     //путь к файлу бд

const db = new Database(dbPath, { verbose: console.log});   //подключение к бд
...
if (pvzCount === 0) {
    ...
    const pvzList = [         //массив
      [1, 'ул. Ленина, 10, ТЦ "Галерея"', "Москва"],
      [2, "пр. Мира, 25, к. 1", "Caнкт-Петербург"],
    ...
    const transaction = db.transaction(() => {
        pvzList.forEach((pvz) => insertPvz.run(...pvz));      //добавление элементов в таблицу 
    });
...
module.exports = db;    //экспорт объектов в бд