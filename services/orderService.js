...
exports.getById = (id) => {     //Функция объединяет данные из трёх таблиц через JOIN
  const order = db
    .prepare(
        `
...
exports.update = (id, data) => {    //проверка существования клиента и ПВЗ перед созданием заказа
...
exports.delete = (id) => {     //проверяет статус заказа и позволяет удалять только заказы со статусом «new» или «canceled»
...
exports.getStats = () => {     // собирает статистику по заказам — сколько заказов в каждом статусе, общую выручку, распределение по пунктам выдачи
